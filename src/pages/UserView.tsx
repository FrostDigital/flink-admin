import { Button, PageHeader, Space, message, Skeleton, Modal, Card, Descriptions } from "antd"
  ...
  useEffect(()=>{
    ...
    let buttons = [];
     if(module?.features.includes("edit")){
      buttons.push( <Button key="2" type="primary" icon={<EditOutlined />} onClick={ ()=>{ history.push("/modules/user/" + moduleId + "/edit/" + userId ) } }>Edit user</Button>)
     }
    buttons.push( <Button key="3" type="primary" icon={<KeyOutlined />} onClick={ ()=>{ history.push("/modules/user/" + moduleId + "/edit/" + userId + "/password") } }>Change password</Button>)
    
    viewUser?.buttons.forEach(b => {
      if(b.url.startsWith("/")){
        buttons.push( <Button key={b.url} type="primary" onClick={ ()=>{ history.push(b.url) } }>{b.text}</Button>)
      }else{
        buttons.push( <Button key={b.url} type="primary" onClick={ ()=>{ window.open(b.url, "_blank") } }>{b.text}</Button>)
      }
        ...
        {viewUser?.data  &&   
          <Descriptions title="User Info" bordered column={1}>
              {Object.keys(viewUser?.data as Object).map((k)=>{
                return <Descriptions.Item label={k} key={k}>{viewUser?.data[k]}</Descriptions.Item>
              })}
        </Descriptions>}
